---
layout: 'blog-page'
title: How to add How to add “Signin With Apple” to your website
published: true
date: "August 17 2020"
description: Use an apple account to autheticate your users.
tags: apple, javascript, node, singin
cover_image: https://s3.amazonaws.com/media.mediapost.com/dam/cropped/2019/06/05/apple-signinbutton-560.JPEG
---
...
1. User presses the "Sign In with Apple" button.
1. A pop up window opens that takes the user to an apple website where they login into their apple account.
1. After successful login apple sends some information about the user back to the window where we initiated the signin process.
1. We capture the data and send it to our server for verification and storing the information in our database.
...
1. Create an App ID.
1. Create a Serve ID.
1. Register the domains for your website.
...
1. Click on Identifiers in the side panel. Then click the plus symbol next to `Identifiers` to create a new App ID.
...
1. Use a descriptive name for your application, I've used `Example Application` here. We'll choose an explicit style Bundle ID for now, use apple's suggested reverse domain style for proper namespacing, the string can be anything but you should stick to convention. I've used `com.example` here.
...
1. Register a service with a proper description, I've used `Example Application Signin` here. For the identifier we'll follow the convention and use `com.example.signin`.
   ![Register a Service ID](https://user-images.githubusercontent.com/18533307/90203633-d8fb4900-ddfe-11ea-8a56-f7c83f22990b.png)

1. Your new Service ID is created, find it by using the dropdown you see on the left.
   ![Switch to service ID](https://user-images.githubusercontent.com/18533307/90203610-cc76f080-ddfe-11ea-9642-1dbeaf2f069d.png)

1. Click on your Service ID that you just created.
   ![Service ID List](https://user-images.githubusercontent.com/18533307/90203637-da2c7600-ddfe-11ea-9bb5-f1bf28f36a45.png)

1. After you hit continue and save click in the identifier again. You'll be greeted with a checkbox tha say `Sign In with Apple`. Click on Configure.
...
1. A modal will popup, register your domain here.
